...
public:
    ...
    Spectrum eval_diffuse(const BSDFContext &ctx,
                  const SurfaceInteraction3f &si,
                  const Vector3f &wo,
                  Mask active) const override {
        PYBIND11_OVERRIDE_PURE(Spectrum, BSDF, eval_diffuse, ctx, si, wo, active);
    }
...
template <typename Ptr, typename Cls> void bind_bsdf_generic(Cls &cls) {
    ...
    cls.def("sample",
        ...
        .def("eval_diffuse",
             [](Ptr bsdf, const BSDFContext &ctx, const SurfaceInteraction3f &si,
                const Vector3f &wo,
                Mask active) { return bsdf->eval_diffuse(ctx, si, wo, active);
             }, "ctx"_a, "si"_a, "wo"_a, "active"_a = true)
        .def("pdf",
             [](Ptr bsdf, const BSDFContext &ctx, const SurfaceInteraction3f &si,
                const Vector3f &wo,
